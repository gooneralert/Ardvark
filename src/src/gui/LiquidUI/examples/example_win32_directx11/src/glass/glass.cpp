...
static ImU32 AccentCol();   // defined in widgets_misc.inc (included at the bottom)
...
void Renderer::Render(ID3D11ShaderResourceView* heavy, ID3D11ShaderResourceView* soft) {
    ...
    static bool s_logged_queue = false;
    if (!s_logged_queue && !queue_.empty()) {
        s_logged_queue = true;
        std::printf("[glass] prims=%d  r0=%.1f (hw=%.1f hh=%.1f mat=%d)  r1=%.1f  r2=%.1f\n",
                    (int)queue_.size(),
                    queue_[0].corner_radius, queue_[0].hw, queue_[0].hh, (int)queue_[0].material,
                    queue_.size() > 1 ? queue_[1].corner_radius : -1.0f,
                    queue_.size() > 2 ? queue_[2].corner_radius : -1.0f);
        std::fflush(stdout);
    }
...
#include "widgets/widgets_misc.inc"   // SidebarNav + SubmitBlob + AccentCol live here
}
